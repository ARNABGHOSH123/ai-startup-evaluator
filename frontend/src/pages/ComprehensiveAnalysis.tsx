...
export default function ComprehensiveAnalysis({ company }: any) {
  ...
  const fundingsAndFinancialsData = company?.funding_and_financials?.funding_and_financials;
  ...
  return (
    <>
      ...
      />
      <ViewCompetitors viewCompetitors={viewCompetitors}
        setViewCompetitors={setViewCompetitors}
        competitorData={competitorData}/>
      <ViewFundings viewFunding={viewFunding}
      ...
      <div className="grid grid-cols-3 gap-6">
        <Card className="rounded-lg border border-border hover:border-primary bg-background">
          ...
          <CardContent>
            ...
            <div className="space-y-2 text-foreground text-xs">
              ...
              <ul className="list-disc list-inside space-y-1">
                {marketData?.TAM && (
                  <li>
                    <span className="font-medium">TAM:</span> {marketData?.TAM}
                  </li>
                )}
                {marketData?.SAM && (
                  <li>
                    <span className="font-medium">SAM:</span> {marketData?.SAM}
                  </li>
                )}
                {marketData?.SOM && (
                  <li>
                    <span className="font-medium">SOM:</span> {marketData?.SOM}
                  </li>
                )}
              </ul>
            </div>
          ...
          {/* <CardDescription className="px-4 pb-4 -mt-2 text-xs text-neutral">
          ...
          <CardContent>
            {competitorData?.domain_wise_competitor_analysis?.length > 0 &&
              competitorData?.geography_wise_competitor_analysis?.length >
                ...
                <div
                  onClick={() =>
                    setMode(mode === "geographical" ? "domain" : "geographical")
                  }
            ...
            {competitorData?.domain_wise_competitor_analysis?.length > 0 &&
              competitorData?.geography_wise_competitor_analysis?.length ===
                0 && (
                <span className={`py-1 text-sm font-semibold text-cardorange`}>
            ...
            {competitorData?.domain_wise_competitor_analysis?.length === 0 &&
              competitorData?.geography_wise_competitor_analysis?.length >
                0 && (
                <span className={`py-1 text-sm text-cardorange font-semibold`}>
              ...
              {(mode?.toString()?.toLowerCase() === "domain"
                ...
                <div key={index} className="space-y-1 px-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {/* Example badges — replace with real attributes */}
        ...
        <Card className="rounded-lg border border-border hover:border-primary bg-background">
          <span className="flex flex-row items-center justify-between">
            <CardHeader className="flex flex-row space-x-2 items-center -mt-3 -ml-2">
              ...
              <CardTitle className="text-sm">Funding and Financials</CardTitle>
            </CardHeader>
            <span
              className="text-xs hover:cursor-pointer text-primary -mt-2 mr-2 font-semibold bg-primary-foreground rounded-xl px-2"
