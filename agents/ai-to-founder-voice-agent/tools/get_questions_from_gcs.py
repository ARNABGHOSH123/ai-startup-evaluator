...
def get_questions_from_gcs(firestore_doc_id: str, folder_name: str = Config.GCP_PITCH_DECK_OUTPUT_FOLDER) -> str:
    """
    Retrieves the questions file content from a Google Cloud Storage bucket.

    Args:
        firestore_doc_id (str): The Firestore document ID to retrieve the questions for.
    ...
    blobs = list(bucket.list_blobs(
        prefix=f"{folder_name}/{firestore_doc_id}/questions/"))
    questions = blobs[0] if len(blobs) > 0 else None
    if not questions:
        return None

    try:
        return json.loads(questions.download_as_string())
